...
int LSM6DS3::init(calData cal, uint8_t address)
{
	...
	writeByte(IMUAddress, LSM6DS3_CTRL1_XL, 0x37);	// Start up accelerometer, set range to +-16g, set output data rate to 52hz, BW_XL bits to 11.
	writeByte(IMUAddress, LSM6DS3_CTRL2_G, 0x3C);	// Start up gyroscope, set range to -+2000dps, output data rate to 52hz.
...
void LSM6DS3::update() {
	...
	accel.accelX = ((float)IMUCount[3] * aRes - 0.00f); 
	accel.accelY = ((float)IMUCount[4] * aRes -	0.01f); 
	accel.accelZ = ((float)IMUCount[5] * aRes -	0.00f);	

	// Calculate the gyro value into actual degrees per second
	gyro.gyroX = ((float)IMUCount[0] * gRes - calibration.gyroBias[0]); //0.33, -3.11, -1.07
	gyro.gyroY = ((float)IMUCount[1] * gRes - calibration.gyroBias[1]);
	gyro.gyroZ = ((float)IMUCount[2] * gRes - calibration.gyroBias[2]);
	// // Calculate the accel value into actual g's per second
	// accel.accelY = -((float)IMUCount[3] * aRes -0.00f );//;calibration.accelBias[0]);
	// accel.accelX = -((float)IMUCount[4] * aRes -0.01f); //calibration.accelBias[1]);
	// accel.accelZ = ((float)IMUCount[5] * aRes -0.00f);//calibration.accelBias[2]);

	// // Calculate the gyro value into actual degrees per second
	// gyro.gyroY = ((float)IMUCount[0] * gRes - 0.33f);//calibration.gyroBias[0]); //0.33, -3.11, -1.07
	// gyro.gyroX = ((float)IMUCount[1] * gRes +3.11f); //calibration.gyroBias[1]);
	// gyro.gyroZ = ((float)IMUCount[2] * gRes +1.07f);// calibration.gyroBias[2]);
